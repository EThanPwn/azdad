...
RegisterServerEvent('es_em:sjl_resurectPlayer')
AddEventHandler('es_em:sjl_resurectPlayer',
  function(sourcePlayerInComa)
    TriggerClientEvent('es_em:cjl_resurectPlayer', sourcePlayerInComa)
  end
)]]

RegisterServerEvent('es_em:sjl_getJobId')
AddEventHandler('es_em:sjl_getJobId',
  function()
    TriggerClientEvent('es_em:cjl_setJobId', source, GetJobId(source))
  end
)

RegisterServerEvent('es_em:sjl_getDocConnected')
AddEventHandler('es_em:sjl_getDocConnected',
  function()
    TriggerEvent("es:getPlayers", function(players)
      local identifier
      local table = {}
      ...
      for i,v in pairs(players) do
        ...
        if (identifier ~= nil) then
          local executed_query = MySQL:executeQuery("SELECT identifier, job_id, job_name FROM users LEFT JOIN jobs ON jobs.job_id = users.job WHERE users.identifier = '@identifier' AND job_id = 15 AND enService = 1", {['@identifier'] = identifier[1]})
      ...
      TriggerClientEvent('es_em:cjl_getDocConnected', source, isConnected)
    end)
  end
)

RegisterServerEvent('es_em:sjl_setService')
AddEventHandler('es_em:sjl_setService',
  function(service)
...
RegisterServerEvent('es_em:sjl_removeMoney')
AddEventHandler('es_em:sjl_removeMoney',
  function()
    TriggerEvent("es:getPlayerFromId", source,
      function(user)
        if(user)then
...
RegisterServerEvent('es_em:sjl_sendMessageToPlayerInComa')
AddEventHandler('es_em:sjl_sendMessageToPlayerInComa',
  function(sourcePlayerInComa)
    TriggerClientEvent('es_em:cjl_sendMessageToPlayerInComa', sourcePlayerInComa)
  end
)
