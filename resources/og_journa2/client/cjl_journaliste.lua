...
local txt = {
  ['fr'] = {
		...
		--['callExpires'] = '<b>~r~SCOOP~s~ <br><br>Attention, l\'appel précèdent a expiré !</b>',
		--['gps'] = 'Un point a été placé sur votre GPS',
		--['res'] = 'Appuyez sur ~g~E~s~ pour réanimer le joueur',
		['notJou'] = 'Vous n\'êtes pas journaliste',
...
local playerInComaIsAJou = false
			...
			if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 100.0) then
				...
				if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 2.0) then
					...
					if (IsControlJustReleased(1, 51)) then
						TriggerServerEvent('es_em:sjl_getJobId')
					end
				end
			end
		end
	...
	function()
		...
		while true do
			...
			if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 100.0) and isInService and jobId == 16 then
				...
				if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 2.0) then
					DisplayHelpText(txt[lang]['getJouheli'])
...
RegisterNetEvent('es_em:sendEmergencyToJous')
AddEventHandler('es_em:sendEmergencyToJous',
	function(reason, playerIDInComa, x, y, z, sourcePlayerInComa)
		local playerServerId = GetPlayerServerId(PlayerId())

		if playerIDInComa == playerServerId then playerInComaIsAJou = true else playerInComaIsAJou = false end

		Citizen.CreateThread(
			function()
				if isInService and jobId == 16 and not playerInComaIsAJou then
...
AddEventHandler('es_em:callTaken',
	function(playerName, playerID, x, y, z, sourcePlayerInComa)
		...
		if playerServerId == playerID then
			TriggerServerEvent('es_em:sjl_sendMessageToPlayerInComa', sourcePlayerInComa)
			StartEmergency(x, y, z, sourcePlayerInComa)
		end
end)]]

RegisterNetEvent('es_em:cjl_setJobId')
AddEventHandler('es_em:cjl_setJobId',
	function(p_jobId)
		jobId = p_jobId
		GetService()
	end
...
function SpawnJouveh()
	...
	local spawned_car = CreateVehicle(vehicle, coords, -1099.238, -256.956, 37.687, true, false)

	SetVehicleOnGroundProperly(spawned_car)
	SetVehicleNumberPlateText(spawned_car, "JOURNA")
	SetPedIntoVehicle(myPed, spawned_car, - 1)
	SetModelAsNoLongerNeeded(vehicle)
	Citizen.InvokeNative(0xB736A491E64A32CF, Citizen.PointerValueIntInitialized(spawned_car))
end

function SpawnJouheli()
	Citizen.Wait(0)
	local myPed = GetPlayerPed(-1)
	local player = PlayerId()
	...
	local plate = math.random(100, 900)
	local coords = GetOffsetFromEntityInWorldCoords(GetPlayerPed(-1), 0, 5.0, 0)
	local spawned_car = CreateVehicle(vehicle, coords, -1047.583, -234.785, 53.511, true, false)
			...
			while not isRes do
				...
				if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), x,y,z, true)<3.0) then
						...
						if (IsControlJustReleased(1, Keys['E'])) then
							...
							Citizen.Wait(8000)
							ClearPedTasks(ped);
	            TriggerServerEvent('es_em:sjl_resurectPlayer', sourcePlayerInComa)
	            isRes = true
	          end
				end
			end
...
function GetService()
	...
	if jobId ~= 16 then
		SendNotification(txt[lang]['notJou'])
		return
	end
	...
	if isInService then
		...
		TriggerServerEvent('es_em:sjl_setService', 0)
	else
		SendNotification(txt[lang]['startService'])
		TriggerServerEvent('es_em:sjl_setService', 1)
	end
