	...
	if isOwnedVehicleSpawned then
		...
		if DecorGetInt(GetVehiclePedIsIn(GetPlayerPed(-1), false), "owner") then
			...
			if DecorGetInt(GetVehiclePedIsIn(GetPlayerPed(-1), false), "owner") == GetPlayerServerId(PlayerId()) then
				...
				TriggerEvent('chatMessage', 'SHOP', {255, 0, 0}, 'Vous avez rangé votre véhicule!')
				isOwnedVehicleSpawned = false
			else
				TriggerEvent('chatMessage', 'SHOP', {255, 0, 0}, 'Vous devez être dans votre véhicule pour le ranger.')
			end
		else
			TriggerEvent('chatMessage', 'SHOP', {255, 0, 0}, 'Vous devez être dans votre véhicule personnel pour le ranger.')
		end
	else
		TriggerEvent('chatMessage', 'SHOP', {255, 0, 0}, 'Vous devez faire apparaître votre véhicule personnel pour le ranger.')
	end
end)
...
Citizen.CreateThread(function()
    while true do
			...
			if(showFixMessage)then
				DisplayHelpText("Tu as ~g~réparé~w~ ton ~b~véhicule~w~!")
			end
					...
					if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 100.0)then
						...
						if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 2.0 and showFixMessage == false)then
							if(not IsPedInAnyVehicle(GetPlayerPed(-1), false))then
								DisplayHelpText("Appuie sur ~INPUT_CONTEXT~ pour accéder au ~b~garage~w~ pour acheter ou sortir un véhicule.")
								...
								if(IsVehicleDamaged(GetVehiclePedIsIn(GetPlayerPed(-1), false)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1))then
									DisplayHelpText("Appuie sur ~INPUT_CONTEXT~ pour réparer ton véhicule.")
									if(IsControlJustReleased(1, 51))then
										showFixMessage = true
										SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1)))
									end
								else
									DisplayHelpText("Tu dois être à pied pour accéder au garage.")
								end
							end
						end
					end
