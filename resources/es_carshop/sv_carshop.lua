...
MySQL:open("127.0.0.1", "gta5_script_carshop", "root", "18020603")
...
AddEventHandler('es_carshop:buyVehicle', function(veh)
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es_roleplay:getPlayerJob', user.identifier, function(job)
			if(veh == "mule" or veh == "benson" or veh == "pounder")then
				if(job)then
			...
			if(veh == "towtruck" or veh == "flatbed")then
				if(job)then
					if(job.job ~= "serviceman")then
						TriggerClientEvent('es_carshop:closeWindow', source)
						TriggerClientEvent('chatMessage', source, "SHOP", {255, 0, 0}, "Tu ne peux acheter ou faire spawn ce véhicule que si tu es ^2dépanneur^0. tape /job serviceman 1 pour devenir ^2dépanneur^0.")
						return
					end
				else
					TriggerClientEvent('es_carshop:closeWindow', source)
					TriggerClientEvent('chatMessage', source, "SHOP", {255, 0, 0}, "Tu ne peux acheter ou faire spawn ce véhicule que si tu es ^2dépanneur^0.")
					return
				end
			end

			if(veh == "ambulance")then
				if(job)then
					if(job.job ~= "ems")then
						TriggerClientEvent('es_carshop:closeWindow', source)
						TriggerClientEvent('chatMessage', source, "SHOP", {255, 0, 0}, "Tu ne peux acheter ou faire spawn ce véhicule que si tu es ^2Ambulancier^0.")
						return
					end
				else
					TriggerClientEvent('es_carshop:closeWindow', source)
			...
			if(vehicle_data[source])then
				for k,v in ipairs(vehicle_data[source]) do
					if(v.owner == user.identifier and veh == v.model)then
						...
						if(v.model == "police" or v.model == "mule" or v.model == "benson" or v.model == "pounder" or v.model == "policeb")then
