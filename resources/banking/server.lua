...
TriggerEvent('es:addCommand', 'solde', function(source, args, user)
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    local bankbalance = bankBalance(player)
    TriggerClientEvent("es_roleplay:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Solde actuel sur votre compte: ~g~$".. bankbalance)
  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
      ...
      if(string.len(rounded) >= 9) then
        ...
        CancelEvent()
          ...
          TriggerClientEvent("es_roleplay:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Déposé: ~g~$".. rounded .." ~n~~s~Nouveau solde: ~g~$" .. new_balance)
  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
      ...
      if(string.len(rounded) >= 9) then
        ...
        if(tonumber(rounded) <= tonumber(bankbalance)) then
          ...
          local new_balance = bankBalance(player)
          TriggerClientEvent("es_roleplay:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Retiré: ~g~$".. rounded .." ~n~~s~Nouveau solde: ~g~$" .. new_balance)
          TriggerClientEvent("banking:updateBalance", source, new_balance)
    ...
    TriggerEvent('es:getPlayerFromId', fromPlayer, function(user)
        ...
        if(string.len(rounded) >= 9) then
          ...
          if(tonumber(rounded) <= tonumber(bankbalance)) then
            withdraw(player, rounded)
            local new_balance = bankBalance(player)
            TriggerClientEvent("es_roleplay:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Transféré: ~r~-$".. rounded .." ~n~~s~Nouveau solde: ~g~$" .. new_balance)
            TriggerClientEvent("banking:updateBalance", source, new_balance)
            TriggerClientEvent("banking:removeBalance", source, rounded)
            TriggerEvent('es:getPlayerFromId', toPlayer, function(user2)
                ...
                TriggerClientEvent("es_roleplay:notify", toPlayer, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Reçu: ~g~$".. rounded .." ~n~~s~Nouveau solde: ~g~$" .. new_balance2)
	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		if (tonumber(user.money) >= tonumber(amount)) then
			...
			TriggerEvent('es:getPlayerFromId', toPlayer, function(recipient)
				...
				TriggerClientEvent("es_roleplay:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Cash donné: ~r~-$".. amount .." ~n~~s~Portefeuille: ~g~$" .. user.money)
				TriggerClientEvent("es_roleplay:notify", toPlayer, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Cash reçu: ~g~$".. amount .." ~n~~s~Portefeuille: ~g~$" .. recipient.money)
			end)
		else
			if (tonumber(user.money) < tonumber(amount)) then
...
AddEventHandler('bank:withdrawAmende', function(amount)
    TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		local new_balance = bankBalance(player)
		TriggerClientEvent("es_roleplay:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Nouveau solde: ~g~$" .. new_balance)
		TriggerClientEvent("banking:updateBalance", source, new_balance)
